...
def build_hanging_cable(device: str) -> tuple[newton.Model, int, list[int], list[int]]:
    ...
    builder.color()
    return builder.finalize(device=device), bodies[-1], cable_joints, bodies[1:]


def simulate_hanging_load(applied_downward_force: float, device: str = "cpu") -> dict[str, np.ndarray | float]:
    model, body, joints, supported_bodies = build_hanging_cable(device)
    solver = newton.solvers.SolverVBD(model, iterations=LOAD_ITERATIONS, rigid_contact_hard=True)
    state_0 = model.state()
    state_1 = model.state()
    control = model.control()

    masses = model.body_mass.numpy()
    mass = float(np.sum(masses[supported_bodies]))
    ...
    for step in range(LOAD_STEPS):
        ...
        tension.append(float(state_1.vbd.cable_tension.numpy()[joints][0]))
    ...
    return {
        ...
        "supported_mass": float(mass),
        "applied_downward_force": float(applied_downward_force),
        "cable_segments": len(supported_bodies) + 1,
    }
...
def _hanging_panel_frame(time: list[float], tension: list[float], expected: float) -> np.ndarray:
    ...
    draw.text((58, 668), "error = VBD tension - static load reference", fill=(91, 104, 120), font=_font(13))
...
def _render_hanging_video(output_path: Path, viewer: ViewerGL) -> None:
    ...
    mass = float(np.sum(model.body_mass.numpy()[supported_bodies]))
    expected_tension = mass * GRAVITY + applied_downward_force

    viewer.set_model(model)
    viewer.set_camera(pos=wp.vec3(0.0, -3.0, -0.25), pitch=-8.0, yaw=90.0)
    ...
    try:
        for frame in range(VIDEO_FPS * VIDEO_SECONDS):
            ...
            tension = float(state_0.vbd.cable_tension.numpy()[joints][0])
