...
def harvest_proxy_wrenches_from_contact_forces_kernel(
    ...
    vbd_to_mj_body_map: wp.array(dtype=int),
    ...
    dynamic (inv_mass > 0).  Excludes proxy-proxy and proxy-static contacts.
    ...
    # Non-proxy body must be dynamic (excludes proxy vs static environment)
...
def set_target_pose_kernel(
    ...
    how the target is computed:
      ...
      GRASP / HOLD_GRASP — close gripper; one-way uses snapshot pos to avoid feedback
    ...
    if task == TaskType.APPROACH.value:
        # Track live capsule + approach stand-off offset; align EE to capsule axis.
        ...
        # Move to grasp pose (no approach offset); still tracks live capsule position.
        ...
        # Close gripper around capsule.  One-way mode uses the snapshot position to
        # avoid a positive feedback loop (gripper chasing sinking capsule).
        grasp_pos_offset = wp.quat_rotate(capsule_quat_prev, capsule_grasp_offset_from_com[arm_idx])
        if grasp_top_bias != 0.0:
            grasp_pos_offset = grasp_pos_offset + grasp_top_bias * top_side_unit_from_capsule_quat(capsule_quat_prev)
        if one_way_mode == 1:
            capsule_pos_prev = wp.transform_get_translation(task_capsule_body_q_prev[arm_idx])
            ee_pos_target[arm_idx] = capsule_pos_prev + grasp_pos_offset
        else:
            ee_pos_target[arm_idx] = capsule_pos + grasp_pos_offset
        ee_quat_target = ee_quat_prev
        t_gripper = 1.0
    elif task == TaskType.EXTRACT.value:
        # Linear pull along capsule +Z at the moment extraction started.
        # ee_pos_target is the FINAL endpoint; the lerp below ramps linearly over time.
...
def apply_gripper_centering_correction_kernel(
    ...
    Active phases:
      - Two-way mode (``two_way_mode=1``): ENGAGE, GRASP, HOLD_GRASP, EXTRACT.
      - One-way mode (``two_way_mode=0``): ENGAGE only — later phases are skipped
        to avoid capsule-to-IK positive feedback loops.
    """
    arm = wp.tid()
...
@wp.kernel(enable_backward=False)
def apply_one_way_gripper_gap_lock_kernel(
    task_schedule: wp.array(dtype=wp.int32),
    task_idx: wp.array(dtype=int),
    vbd_body_q: wp.array(dtype=wp.transform),
    finger_proxy_body_indices: wp.array2d(dtype=int),
    gripper_dof_indices: wp.array2d(dtype=int),
    mujoco_joint_q: wp.array(dtype=float),
    stop_gap: float,
    # in/out
    lock_active: wp.array(dtype=wp.int32),
    lock_values: wp.array2d(dtype=wp.float32),
    gripper_target: wp.array2d(dtype=wp.float32),
):
    """Freeze one-way gripper joints once finger gap reaches the stop threshold [m].

    In one-way mode the gripper has no force feedback, so it can over-close and
    cause deep penetration.  This kernel monitors the Euclidean distance between the
    two VBD finger proxy bodies and, once it drops below ``stop_gap``, captures the
    ...
    active = lock_active[arm]

    # Trigger lock: capture current MuJoCo gripper joint positions when gap first
    # drops below the threshold during GRASP or HOLD_GRASP.
    if active == 0 and (task == TaskType.GRASP.value or task == TaskType.HOLD_GRASP.value):
        gap = wp.length(wp.transform_get_translation(vbd_body_q[f1]) - wp.transform_get_translation(vbd_body_q[f0]))
        if gap <= stop_gap:
            d0 = gripper_dof_indices[arm, 0]
            d1 = gripper_dof_indices[arm, 1]
            if d0 >= 0 and d1 >= 0 and d0 < mujoco_joint_q.shape[0] and d1 < mujoco_joint_q.shape[0]:
                lock_values[arm, 0] = wp.float32(mujoco_joint_q[d0])
                lock_values[arm, 1] = wp.float32(mujoco_joint_q[d1])
                lock_active[arm] = 1
                active = 1

    # Override gripper targets with frozen values while lock is active.
    if active == 1:
        gripper_target[arm, 0] = lock_values[arm, 0]
        gripper_target[arm, 1] = lock_values[arm, 1]
...
class Example:
    def __init__(self, viewer, num_worlds=1, args=None):
        ...
        self.enable_proxy_coupling = self.enable_one_way_coupling or self.enable_two_way_coupling
        self.exchange_forces_every_substep = True

        # How to reconstruct SolverVBD proxy previous pose each substep:
        #   "snapshot"           — store proxy prev pose and reuse it (default)
        #   "integrate_velocity" — reconstruct from current VBD velocity
        ...
        # ----------------------------------------------------------------
        # Approach offsets [m]: lateral (Y) and vertical (Z) stand-off from capsule COM.
        ...
        # Preferred bias for symmetry tuning: world-up direction projected orthogonal to capsule axis.
        # Positive values nudge grasp target toward the "top" side of the capsule.
        self.sm_grasp_top_bias = float(getattr(args, "grasp_top_bias", 0.002)) if args is not None else 0.002
        ...
        one_way_stop_gap_arg = float(getattr(args, "one_way_stop_gap", -1.0)) if args is not None else -1.0
        self.one_way_grasp_stop_gap = (
            one_way_stop_gap_arg
            if one_way_stop_gap_arg > 0.0
            else (
                2.0 * self.capsule_radius
                + 2.0 * self.vbd_proxy_thickness
                + 2.0 * self.vbd_capsule_thickness
                + self.vbd_default_contact_margin
                + self.vbd_capsule_contact_margin
            )
        )
    ...
    def _create_collision_pipeline(self, collision_mode: CollisionMode, args=None):
        ...
        if collision_mode == CollisionMode.MUJOCO:
            ...
            return newton.CollisionPipeline(self.mujoco_model, reduce_contacts=True, broad_phase_mode="explicit")
        elif collision_mode == CollisionMode.NEWTON_HYDROELASTIC:
    ...
    def setup_robot_builder(self):
        ...
        # Setting some gains to make the robot stay upright.
    ...
    def _create_proxy_bodies(self, vbd_builder):
        ...
        One-way mode uses very large mass (effectively kinematic) so VBD forward
        integration has negligible effect; two-way mode preserves the MuJoCo mass
        (optionally scaled by ``proxy_mass_scale``).
        """
        if self.verbose:
        ...
        # defaults, with overrides for mu and thickness specific to proxies).
        ...
        for mj_body_id in range(self.mujoco_model.body_count):
            ...
            scaled_body_mass = body_mass * self.proxy_mass_scale
            ...
            # One-way: very large mass so VBD forward integration has negligible effect
            # (positions are overwritten from MuJoCo each step anyway).
            proxy_mass = 0.0 if self.enable_one_way_coupling else scaled_body_mass

            proxy_body_id = vbd_builder.add_body(
                xform=initial_xform,
                mass=proxy_mass,
                key=f"proxy_{body_key}",
            )
    ...
    def _init_coupling_buffers(self):
        ...
        if self.enable_two_way_coupling:
            # Coupling wrench/force buffers on MuJoCo bodies (two-way only).
    ...
    def _setup_vbd_world_and_coupling(self, args):
        ...
        vbd_builder.add_shape_mesh(
            body=-1,
            mesh=mesh,
            xform=wp.transform(p=mesh_pos_a, q=mesh_quat),
            cfg=vbd_ground_cfg,
        ...
        vbd_builder.add_shape_mesh(
            body=-1,
            mesh=mesh,
            xform=wp.transform(p=mesh_pos_b, q=mesh_quat),
            cfg=vbd_ground_cfg,
        ...
        # Create proxy bodies for both coupling modes.
        # - one-way: proxies are kinematic colliders in VBD (no force feedback to MuJoCo)
        # - two-way: proxies are dynamic and exchange forces with MuJoCo
        ...
        # Initialize proxy/coupling buffers when either coupling mode is active.
        if self.enable_robot and self.enable_proxy_coupling:
    ...
    def setup_state_machine(self):
        ...
        # Time limits [s] per task entry (DONE gets a large sentinel value).
        ...
        oz = self.sm_approach_offset_z
        self.approach_offsets = wp.array(
            [
                wp.vec3(0.0, -oy, oz),  # Right arm -> Capsule A
                wp.vec3(0.0, oy, oz),  # Left arm  -> Capsule B
            ],
            dtype=wp.vec3,
        )
        ...
        # Grasp orientations: 180° Y-flip, ±90° Z-yaw, then -22.5° Y-tilt.
        tilt = -wp.pi / 8.0  # -22.5° [rad]
        ...
        # Centering correction: proportional corrections that keep the capsule
        # centered between the gripper fingers (closing-axis) and aligned along the
        # capsule's medial axis.
        self.gripper_centering_enable = True
        self.gripper_centering_k = 0.4  # closing-axis correction gain
        self.gripper_axis_centering_k = 0.8  # medial-axis correction gain
        self.gripper_centering_max_step = 0.003  # [m] per frame clamp for stability
        ...
        self.one_way_gripper_lock_active = wp.zeros(self.num_arms, dtype=wp.int32)
        self.one_way_gripper_lock_values = wp.zeros(shape=(self.num_arms, 2), dtype=wp.float32)
    ...
    def _start_auto_mode(self):
        ...
        self.one_way_gripper_lock_active.zero_()
        self.one_way_gripper_lock_values.zero_()
    ...
    def simulate(self):
        ...
        if self.exchange_forces_every_substep:
            ...
            shared_substep_dt = self.frame_dt / shared_substep_count
                ...
                if self.enable_two_way_coupling and self.enable_robot and hasattr(self, "proxy_vbd_body_ids_array"):
                    ...
                    if (
                        ...
                        c_b0, c_b1, c_p0w, c_p1w, c_f_b1, c_count = self.vbd_solver.collect_rigid_contact_forces(
                            vbd_state_1, self.vbd_contacts, shared_substep_dt
                        )
                        wp.launch(
            ...
            # ------------------------------------------------------------
            mujoco_substep_dt = self.frame_dt / mujoco_substep_count
            dt_vbd = self.frame_dt / vbd_substep_count
            ...
            if self.enable_two_way_coupling and self.enable_robot and hasattr(self, "proxy_mj_body_ids_array"):
                self.proxy_forces_prev.assign(self.proxy_forces)
                wp.launch(
                    subtract_proxy_forces_kernel,
                    dim=len(self.proxy_body_ids),
                    inputs=[
            ...
            for _ in range(vbd_substep_count):
                ...
                if self.enable_two_way_coupling and self.enable_robot and hasattr(self, "proxy_vbd_body_ids_array"):
                    ...
                    if (
                        ...
                        c_b0, c_b1, c_p0w, c_p1w, c_f_b1, c_count = self.vbd_solver.collect_rigid_contact_forces(
                            vbd_state_1, self.vbd_contacts, dt_vbd
                        )
        ...
        if self.auto_mode:
            ...
            wp.launch(
                ...
                inputs=[
                    ...
                    int(self.enable_one_way_coupling),
                ],
                outputs=[
            ...
            if self.gripper_centering_enable:
                wp.launch(
                    ...
                    inputs=[
                        ...
                        int(not self.enable_one_way_coupling),
                    ],
            ...
            # In one-way mode, lock gripper targets once the finger gap falls below threshold.
            if self.enable_one_way_coupling:
                wp.launch(
                    apply_one_way_gripper_gap_lock_kernel,
                    dim=self.num_arms,
                    inputs=[
                        self.sm_task_schedule,
                        self.sm_task_idx,
                        self.vbd_state_0.body_q,
                        self.sm_finger_proxy_body_indices,
                        self.sm_gripper_dof_indices,
                        self.mujoco_state_0.joint_q,
                        self.one_way_grasp_stop_gap,
                    ],
                    outputs=[
                        self.one_way_gripper_lock_active,
                        self.one_way_gripper_lock_values,
                        self.sm_gripper_target,
                    ],
                )

            # Copy (num_arms, 2) gripper targets to flat gripper_targets array.
    ...
    def step(self):
        ...
        if self.graph_sim:
            if self.exchange_forces_every_substep:
                step_count = max(int(self.mujoco_substeps), int(self.vbd_substeps))
                if step_count % 2 == 1:
                    if self.enable_robot:
                        self.mujoco_state_0.assign(self.mujoco_state_1)
                    self.vbd_state_0.assign(self.vbd_state_1)
            else:
                if self.enable_robot and int(self.mujoco_substeps) % 2 == 1:
                    self.mujoco_state_0.assign(self.mujoco_state_1)
                if int(self.vbd_substeps) % 2 == 1:
                    self.vbd_state_0.assign(self.vbd_state_1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.002,
        help="Additional grasp target offset in meters toward world-up, projected orthogonal to capsule axis.",
    )
    parser.add_argument(
        "--one-way-stop-gap",
        type=float,
