...
4 N*m net torque. This example uses the compliant-ALM rigid path; legacy AVBD
uses a smooth near-rest approximation instead of exact static sticking.
...
class Example:
    ...
    def __init__(self, viewer, args):
        ...
        self.solver = newton.solvers.SolverVBD(self.model, iterations=10, rigid_compliant_alm=True)
    ...
    def test_final(self):
        """Check exact sticking and bidirectional sliding when run with --test."""
        velocity = self.joint_qd.numpy()
        if not np.isfinite(velocity).all():
            raise ValueError("Non-finite joint-friction state")
        if np.max(np.abs(velocity[1:3])) > 1.0e-5:
            raise ValueError(f"Sub-threshold hinges failed to stick: qd={velocity[1:3]}")
        if velocity[0] >= -0.1 or velocity[3] <= 0.1:
            raise ValueError(f"Above-threshold hinges failed to slide bidirectionally: qd={velocity[[0, 3]]}")
